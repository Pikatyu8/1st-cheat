...
hack = {
    ...
    bindKeys: function() {
            document.addEventListener('keydown', function(event) {
                if (event.key.toLowerCase() === 'z' && !event.repeat) { // Проверка event.repeat
...
hack.getters.mode.playerMovement = function(e) {
    ...
    if (hack.getters.mode.moveLeft) {
      ...
      if (hack.keyBindings.isCPressed && hack.getters.mode.moveDown && currentTime - (hack.playerMoveData.lastDashTime || 0) >= dashCooldown && !hack.playerMoveData.isDashing) {
        hack.playerMoveData.lastDashTime = currentTime;
        hack.playerMoveData.dashStartPosition = hack.getters.me.getY();
      ...
      if (hack.keyBindings.isCPressed && hack.getters.mode.moveUp && currentTime - (hack.playerMoveData.lastDashTime || 0) >= dashCooldown && !hack.playerMoveData.isDashing) {
        hack.playerMoveData.lastDashTime = currentTime;
        hack.playerMoveData.dashStartPosition = hack.getters.me.getY();
    ...
    if (hack.keyBindings.isCPressed && currentTime - (hack.playerMoveData.lastDashTime || 0) >= dashCooldown && !hack.playerMoveData.isDashing) {
        hack.playerMoveData.lastDashTime = currentTime;
        hack.playerMoveData.dashStartPosition = hack.getters.me.getX();
    ...
    if (hack.playerMoveData.isDashingDown) {
        ...
        if (currentTime >= hack.playerMoveData.dashEndTime) {
            ...
            if (!hack.vars.immIsOn) {hack.getters.me.me = true}
            if (!hack.vars.modeIsOn) {hack.getters.me.p.invMass = 1}
        }
        return;
    }
...
function isGrounded() {
  hack.vars.isGround = false
    ...
    if (isGrounded()) {
        hack.playerMoveData.isDoubleJumpAllowed = true; // Разрешить двойной прыжок после приземления
        ...
        hack.getters.velocity[1] = 8;
        hack.playerMoveData.jumps++;
        hack.keyBindings.isZPressed = false;
        hack.playerMoveData.isDoubleJumpAllowed = false; // Запретить двойной прыжок после его использования
    }
