  ...
  document.body.onkeydown = (event) => {
    ...
    switch (keyCode) {
      ...
      case 113:
        ...
        break;
      case 65:
        if (isGrounded() && hack.getters.me.p.gravityScale == 1) {
...
function isGrounded() {
    ...
    const isNegativeGravity = hack.getters.me.p.gravityScale < 0;
    
    // Определяем направление проверки
    const verticalOffset = isNegativeGravity ? -1 : 1;
    const checkYPosition = isNegativeGravity ? hack.getters.me.getY() - 49 : hack.getters.me.getY() + 49;

    for (let i = 0; i < 31; i++) {
        const o = hack.getters.me.getX() - 15 + i * (30 / 29);
        hack.getters.ray.from = [hack.getters.physics.xAxis(o, 0), hack.getters.physics.yAxis(checkYPosition, 0)];
        hack.getters.ray.to = [hack.getters.physics.xAxis(o, 0), hack.getters.physics.yAxis(checkYPosition + verticalOffset, 0)];
