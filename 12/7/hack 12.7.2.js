...
document.body.insertAdjacentHTML("beforebegin", `
  <div id="controlPanel" style="display: inherit; width: auto; position: fixed; bottom: 0px; left: 0px; height: auto; text-align: left; font-size: 14px; background: rgb(0, 0, 0); color: rgb(255, 255, 255); opacity: 0.7; padding: 2px 2px;">
    <div>
      <span>modeIsOn: </span>
      <button id="modeBtn" style="background: rgba(255, 255, 255, 0.7); color: black;">${hack.vars.modeIsOn}</button>
    </div>
    <div>
      <span>immIsOn: </span>
...
const updateButtonStates = () => {
  document.getElementById("modeBtn").innerText = hack.vars.modeIsOn;
...
function isGrounded() {
    ...
    const verticalOffset = hack.getters.me.p.gravityScale < 0 ? -50 : 50; // Определяем смещение в зависимости от гравитации
    const checkYPosition = hack.getters.me.p.gravityScale < 0 ? meY - 45 : meY + 45;

    for (let i = 0; i < 121; i++) {
        const o = meX - 15 + i * (30 / 120);
        const s = checkYPosition; // Используем проверяемую позицию
        const u = s + verticalOffset; // Смещение на 50 или -50
