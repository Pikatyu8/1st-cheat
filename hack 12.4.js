...
hack.getters.mode.playerMovement = function(e) {
    ...
    if (hack.getters.mode.moveLeft) {
      ...
      if (hack.keyBindings.isCPressed && hack.getters.mode.moveDown && currentTime - (hack.playerDirection.lastDashTime || 0) >= dashCooldown && !hack.playerDirection.isDashing) {
        hack.playerDirection.lastDashTime = currentTime;
        hack.playerDirection.dashStartPosition = hack.getters.me.getY();
        hack.playerDirection.isDashingDown = true;
      ...
      if (hack.keyBindings.isCPressed && hack.getters.mode.moveUp && currentTime - (hack.playerDirection.lastDashTime || 0) >= dashCooldown && !hack.playerDirection.isDashing) {
        hack.playerDirection.lastDashTime = currentTime;
        hack.playerDirection.dashStartPosition = hack.getters.me.getY();
        hack.playerDirection.isDashingUp = true;
        hack.playerDirection.dashDuration = (dashDistance / dashSpeed) * 1000;
        hack.playerDirection.dashEndTime = currentTime + hack.playerDirection.dashDuration;
    }
    ...
    if (hack.playerDirection.isDashingDown) {
        ...
        if (currentTime >= hack.playerDirection.dashEndTime) {
            hack.playerDirection.isDashingDown = false;
      ...
      if (hack.playerDirection.isDashingUp) {
        hack.getters.mode.player.gpData.p.velocity[1] = dashSpeed
        hack.getters.mode.player.gpData.p.velocity[0] = 0;
        hack.getters.me.me = void 0
        hack.getters.me.p.invMass = 0
        if (currentTime >= hack.playerDirection.dashEndTime) {
            hack.playerDirection.isDashingUp = false;
            hack.getters.mode.player.gpData.p.velocity[1] = 0;
            if (!hack.vars.immIsOn) {hack.getters.me.me = true}
            if (!hack.vars.modeIsOn) {hack.getters.me.p.invMass = 1}
        }
